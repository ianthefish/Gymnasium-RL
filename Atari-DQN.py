...
plot_folder = "plots-DQN"
video_folder = "video-DQN"
model_folder = "model-DQN"
...
q_network = QNetwork(input_channels, num_actions).to(device)
target_network = QNetwork(input_channels, num_actions).to(device)
...
while global_step < total_train_frames:
    ...
    for t in range(10000): 
        ...
        if len(buffer) >= train_start and global_step % train_freq == 0:
            ...
            with torch.no_grad():
                next_q_target = target_network(b_s_).max(1, keepdim=True)[0]
    ...
    if (ep + 1) % 100 == 0:
        torch.save(q_network.state_dict(), os.path.join(model_folder, f"dqn_enduro_ep{ep+1}.pth"))

    ep += 1

torch.save(q_network.state_dict(), os.path.join(model_folder, "dqn_enduro_final.pth"))
