    ...
    def get_batches(self, batch_size):
        ...
        for start in range(0, self.ptr, batch_size):
            ...
            yield (
                ...
                torch.tensor(self.actions[b], dtype=torch.int64),
...
def train():
    ...
    optimizer_policy = optim.Adam(model.actor.parameters(), lr=LEARNING_RATE_POLICY)
    optimizer_critic = optim.Adam(model.critic.parameters(), lr=LEARNING_RATE_CRITIC)

    if ANNEAL_LR:
        scheduler_policy = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_policy, T_max=NUMBER_OF_EPISODES)
        scheduler_critic = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_critic, T_max=NUMBER_OF_EPISODES)

    buffer = RolloutBuffer(NUMBER_OF_STEPS, obs_dim)
    ...
    for update in range(1, NUMBER_OF_EPISODES + 1):
        ...
        for _ in range(NUMBER_OF_STEPS):
            ...
            lp_np, v_np = lp.cpu().item(), v.cpu().item()
            a_np = a.item()
        ...
        print(f"[Episode {update:03d}] Reward: {rollout_reward:.2f} | Policy Loss: {total_policy:.4f} | Value Loss: {total_value:.4f}")
